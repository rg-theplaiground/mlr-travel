...
import { Hotel as HotelIcon, ArrowRightLeft, Search, Loader2, AlertTriangle, ArrowLeft } from 'lucide-react';
...
interface BookFlightProps {
  onNavigateToCheckout?: (item: any) => void;
  initialSearchData?: { destination: string; date?: Date } | null;
  initialTab?: string;
}

export const BookFlight: React.FC<BookFlightProps> = ({ onNavigateToCheckout, initialSearchData }) => {
  // Always 'hotels'
  const activeTab = 'hotels';

  // View State
  const [viewState, setViewState] = useState<'search' | 'searching' | 'results'>('search');
  const [errorMessage, setErrorMessage] = useState('');

  // Search Configuration
  const [destination, setDestination] = useState('San Diego');
    ...
    try {
      ...
      if (currentSearchId === lastSearchId.current) {
          if (data && data.GetHotelAvailRS?.HotelAvailInfos?.HotelAvailInfo) {
              const hotels: Hotel[] = data.GetHotelAvailRS.HotelAvailInfos.HotelAvailInfo.map((h: any, index: number) => {
                ...
                return {
                  ...
                  amenities: h.HotelInfo.Amenities?.Amenity?.map((a: any) => a.Description) || [],
