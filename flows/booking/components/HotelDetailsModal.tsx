...
export const HotelDetailsModal: React.FC<HotelDetailsModalProps> = ({ hotel, onClose, onBook, searchCriteria }) => {
  const [content, setContent] = React.useState<HotelContentDTO | null>(null);
  const [loading, setLoading] = React.useState(false);
  const [activeImageIndex, setActiveImageIndex] = React.useState(0);
  const [verifyingPrice, setVerifyingPrice] = React.useState(false);
  const [priceChangeData, setPriceChangeData] = React.useState<{ PriceChange?: boolean; HotelRateInfo?: { RateInfos?: { ConvertedRateInfo?: { AverageNightlyRate?: string }[] } } } | null>(null);
