...
import {
  X, Wifi, Zap, ArrowRight,
  Check, Info, Armchair, ArrowLeft, Loader2, AlertCircle, ChevronDown
} from 'lucide-react';
...
export const FlightDetailsModal: React.FC<FlightDetailsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = React.useState<'details' | 'fares'>('fares');
  ...
  const [selectedFareType, setSelectedFareType] = React.useState<string | null>(null);
  ...
  const handleSeatConfirm = (_seats: string[]) => {
...
interface FlightSegmentDetailProps {
  airline: string;
  airlineCode: string;
  flightNumber: string;
  origin: string;
...
const FlightSegmentDetail: React.FC<FlightSegmentDetailProps> = ({ airline, airlineCode, flightNumber, origin, destination, departTime, arriveTime, duration, aircraft, seatPitch, isLast }) => (
...
const FareCardDesktop: React.FC<FareCardProps> = ({ title, price, features, unavailable, onSelect, recommended, isLoading }) => (
  <div className={`
...
interface FareCardMobileProps {
  title: string;
  price: number;
  features: string[];
  comparison: { label: string; value: string; isGood: boolean }[];
  onSelect: () => void;
  recommended?: boolean;
  isLoading?: boolean;
}

const FareCardMobile: React.FC<FareCardMobileProps> = ({ title, price, features, comparison, onSelect, recommended, isLoading }) => {
      ...
      <div className="p-6">
        ...
        `}>
          <div className="pt-4 border-t border-stone-800 space-y-3">
            {comparison.map((item: { label: string; value: string; isGood: boolean }, i: number) => (
