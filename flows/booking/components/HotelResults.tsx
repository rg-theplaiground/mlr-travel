...
export const HotelResults: React.FC<HotelResultsProps> = ({ hotels, isLoading, onSelectHotel }) => {
  const [filterType, setFilterType] = React.useState<'all' | 'bundle' | 'stay'>('all');
...
const FilterTab: React.FC<{ label: string; icon: React.ComponentType<{ size?: number; className?: string }>; active: boolean; onClick: () => void }> = ({ label, icon: Icon, active, onClick }) => (
  <button
    onClick={onClick}
    className={`
